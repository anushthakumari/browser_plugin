...
*/
		...
		// Make a POST request to the API with the links
		fetch("http://localhost:5000/classify", {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
			body: JSON.stringify({ features: result }),
		})
			.then((response) => response.json())
			.then((data) => {
				console.log("API response:", data);
				sendResponse({ links: links, apiResponse: data });
				chrome.storage.local.set({ apiResponse: data });
			})
			.catch((error) => {
				console.error("Error making API request:", error);
				sendResponse({ error: "Failed to make API request" });
			});
